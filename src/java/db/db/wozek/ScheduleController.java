...
public class ScheduleController implements Serializable {
    ...
    @EJB
    private db.db.wozek.ScheduleFacade ejbFacade;
    private List<Schedule> items = null;
    private Schedule selected;

    public List<Schedule> getItemsbyklient() {
        HttpSession session = SessionBean.getSession();
        if (items == null) {
            items = getFacade().findq("select c from Schedule c where c.klient=:klient", "klient", session.getAttribute("uzytkownik").toString());
        }
        return items;
    }
