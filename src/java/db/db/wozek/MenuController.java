...
public class MenuController implements Serializable {
    ...
    @EJB
    private db.db.wozek.MenuFacade ejbFacade;
...
public void init() {
  ...
  String role = new commonbean().getsess("role");

    //role a is for admin will get the variable from session role;
    for(Menu r: ejbFacade.findq("select m from Menu m where m.role=:role","role",role))
    { 
        if(r!=null)
    {
    ...
    quit.setUrl("http://www.google.pl");
